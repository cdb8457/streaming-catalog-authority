...
malformed-but-plausible reports the green checks alone would accept. It reads parsed JSON only; it performs
...
`UNKNOWN_KEY`, `REPORT_STATUS_INVALID`, `REPORT_DIGEST_INVALID`), or `NO_REPORTS` for empty input. Output
carries only report ids, booleans, and problem codes plus a `reportSchemaDigest`.
...
- `test/promotion-report-schema.ts` — 5 tests: all ten live report types validate, each violation class
  (unknown/missing key, bad status/digest, unknown report), fixed-literal drift, empty input, and a
  spawned CLI run.
