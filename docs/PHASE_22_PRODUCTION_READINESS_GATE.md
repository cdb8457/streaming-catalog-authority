...
| 3 | **KEK rotation (O5)** — rewrap tooling + preflight; managed custody/scheduling automation | **met (tooling) / deferred (O5 automation open)** | `docs/PHASE_17_KEK_ROTATION_READINESS.md`; `ops:rewrap-kek` (mutate) + `ops:rewrap-kek -- --plan` (preflight, `mutates:false`); `docs/PHASE_30_KEK_EVIDENCE_PREFLIGHT.md`; `ops:kek-evidence-preflight`; doctor `production-gate-o5-managed-kek` (WARN); CI: `test/kek-rewrap.ts`,`test/kek-evidence-preflight.ts`; `docs/PHASE_5_RUNBOOK.md` §5 | Redaction-safe rewrap **plan** (`needsRewrap`/`alreadyCurrent`/`total`), rotation record, custody/scheduling descriptor report |
...
Phase 30 adds `docs/PHASE_30_KEK_EVIDENCE_PREFLIGHT.md` and `ops:kek-evidence-preflight` as a
redaction-safe one-file descriptor JSON preflight for O5 managed KEK custody/scheduling evidence. It
prepares evidence review only; it does not rotate keys, run age, inspect key files, install
scheduling, contact live services, prove managed custody, or close O5.
...
`docs/PHASE_25_READINESS_REHEARSAL.md`) and `npm run ops:evidence-rehearsal` (documented in
`docs/PHASE_26_EVIDENCE_REHEARSAL.md`) as static rehearsals of the package shape before the real
