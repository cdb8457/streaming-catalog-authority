...
CATALOG_AUTHORITY_APPDATA_DIR="$SMOKE_APPDATA" OPERATOR_UI_HOST_PORT="$SMOKE_PORT" \
  docker compose -p "$SMOKE_PROJECT" -f docker-compose.unraid.runtime.yml run -T --interactive=false --rm ops ops:init
CATALOG_AUTHORITY_APPDATA_DIR="$SMOKE_APPDATA" OPERATOR_UI_HOST_PORT="$SMOKE_PORT" \
  docker compose -p "$SMOKE_PROJECT" -f docker-compose.unraid.runtime.yml up -d app
CATALOG_AUTHORITY_APPDATA_DIR="$SMOKE_APPDATA" OPERATOR_UI_HOST_PORT="$SMOKE_PORT" \
  docker compose -p "$SMOKE_PROJECT" -f docker-compose.unraid.runtime.yml run -T --interactive=false --rm ops --silent \
