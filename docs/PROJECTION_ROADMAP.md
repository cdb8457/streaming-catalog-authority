...
| **Projection Phase 3** | **The reliability loop**: the lifecycle failures Phase 2 hardened, done with **all three real consumers attached** over a **real provider** — six arms, six cycles a run, three consecutive fresh runs, every threshold predeclared in `docs/PROJECTION_PHASE_3_RELIABILITY_LOOP.md` and committed **before** the first measured run. **OPEN, and blocked on an operator input rather than on a defect**: the provider has rotated the CDN origin it hands back out of the operator’s `allowedOrigins`, so the daemon refuses every resolved URL and every read fails EIO — the egress allowlist doing exactly what it exists for, and the **independent** Phase 1 TorBox gate fails identically on the same host. §11 of that document is the blocker and names the one-line authority that clears it. **What it has already established, on the real Unraid host:** all three media servers direct-playing the operator’s real object through the production mount (36/30/30 decoded seconds, startup 1.4–1.7 s, zero stalls), a cold three-way concurrent scan observed with all three in flight **with no barrier to rendezvous at**, and the operator’s four approved windows digest-matched through the mount **and inside each server’s own container**. **Nine gate defects, none in the product, no threshold moved.** It also produced the tranche’s one durable product-contract change: **§11 of the Phase 0 contract, consumer attachment** — a consumer must bind the projected path **before** the daemon first mounts there, demonstrated 8/0 on all three real servers with a control that must fail. **Arms A2–A6 have never completed a cycle.** |
