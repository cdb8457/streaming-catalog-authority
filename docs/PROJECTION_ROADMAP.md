...
| **Projection Phase 6** | **The deployable alpha**: one canonical Unraid operator profile and command set, and the decision Phase 5 §5.1 deferred — **bounded automatic recovery**, reason-aware, budgeted, durable, and refusing anything that is not the daemon's own mount. **CLOSED** on the real Unraid host: `go:recovery-gate:three` completed **three consecutive fresh runs, exit 0, zero skips — 42 arm verdicts, 42 pass, 0 fail, 0 skip**, fourteen arms in every run, from image `sha256:a5f12b92d80464a6e3e280498f22a3bd86e732718cee554b549c6ef58e53aef9`; the alpha install matrix passed **11 of 11 arms** driving the shipped operator command; the seven-gate regression matrix (`mount-health`, `mount-truth`, `serve-death`, `stale-mount`, `sustained-outage` three times each, plus `publisher-mount` and `rclone-comparison`) passed with **zero failures and zero skips**; and the narrow real-provider acceptance passed against a real TorBox account and a real CDN — **7 reads, 0 problems** across the corpus's declared windows, with lease reuse and write refused for both uids. **Provider-free by construction in every recovery arm.** **The arm it exists for is a REFUSAL:** a foreign overlay on the mount point is named `refuse-foreign-mount` / `inspect-mount-owner` and is **asserted still mounted** after a whole sustain window and cooldown, with nothing spent — because in every containerised topology this daemon ships in, the likeliest foreign mount is the operator's own bind, and `--auto-remount` unmounted it once already. **Nineteen defects across thirteen attempts, and EIGHT of them were in the product** — including the one that mattered most: `NewProbeCache` swept the durable recovery ledger out of the one directory the operator contract requires to be durable, so the lockout that makes an infinite restart loop unreachable **did not survive a restart**. `RC11` measured it as `stateAfterRestart=idle attempts=0`. **No threshold moved.** **What it does NOT close:** no G-number; no media server was in any Phase 6 run, so the three-server evidence is still Phase 1's and Phase 3's; one host; and it adds no provider — `rclone` is not the architecture, ADR-002 is untouched, and there is no Real-Debrid or Usenet support, only a named contract for each. **The readiness decision is GO for a rough-edged alpha on exactly one measured combination**, with six rough edges stated in §9 rather than discovered. |
