...
| **Projection Phase 2** | **Mount hardening.** Three failure modes of the daemon's own mount lifecycle, each with a gate: a serve loop killed under a **living** process, the **corpse** a dead process left at the mountpoint, and a provider outage that outlasts the circuit breaker's budget. Gate definitions and run records in `docs/PROJECTION_PHASE_2_MOUNT_HARDENING.md`. **Open — 9 of the 9 fresh runs have happened** (three gates, each closed only by three consecutive fresh runs via its `:three` wrapper). **All three gates have run 3/3 consecutive fresh times on the real Unraid host**, on frozen commit `e54d2fd`, exit 0, no skips, with the host's container, network, volume and mountpoint counts identical before and after every sequence. **The row stays Open** until the multi-frontend harness has also produced figures and this tranche has been reviewed as a whole; nine green runs are the gate evidence, not the tranche's closure. Running them for the first time is also what found the tranche's most serious defect: `--auto-remount` unmounted the operator's bind and recovered **for the daemon and for nobody else** — the daemon logged success and no consumer could see a file. That is fixed, and the sequences were restarted from run 1 on the corrected tree. A gate existing is not a gate passing — the sentence the row above spent four dispatches earning — and neither is a gate that has run twice. Alongside them, and counting toward none of those nine, sits the **multi-frontend comparison harness** (`docs/PROJECTION_PHASE_2_RCLONE_BAKEOFF.md`): it extends G22's measurement across `projectiond` and two rclone cache policies under three manufactured provider failures. It is **a harness and not a gate** — no pass threshold, no `:three` wrapper, no G-number — and its own run record in that document is the authority on whether it has produced figures. |
