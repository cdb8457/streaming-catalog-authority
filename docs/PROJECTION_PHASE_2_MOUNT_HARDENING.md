...
|---|---|---|---|---|---|
| 1/3 | **Unraid `tower`** | **22** | 0 | 0 | PASS in 15,798 ms |
| 2/3 | **Unraid `tower`** | **22** | 0 | 0 | PASS in 15,602 ms |
| 3/3 | **Unraid `tower`** | **22** | 0 | 0 | PASS in 15,756 ms |

> **RUN 3/3, exit 0**, 2026-08-09T18:43:01−05:00 → 18:43:49−05:00 (48 s wall), via
...
> `c5a07bdc8744050e22336e999264b68e3442a78dd4777bdae966a1fb5156423e` (1603 tracked files, byte-identity
...
| Run | Host | Confirmed checks | Failed | Skipped | Evidence |
|---|---|---|---|---|---|
| 1/3 | **Unraid `tower`** | **16** | 0 | 0 | PASS in 17,061 ms |
| 2/3 | **Unraid `tower`** | **16** | 0 | 0 | PASS in 16,872 ms |
| 3/3 | **Unraid `tower`** | **16** | 0 | 0 | PASS in 16,892 ms |

> **RUN 3/3, exit 0**, 2026-08-09T18:43:49−05:00 → 18:44:40−05:00 (51 s wall), via
...
| Run | Host | Confirmed checks | Failed | Skipped | Evidence |
|---|---|---|---|---|---|
| 1/3 | **Unraid `tower`** | **32** | 0 | 0 | PASS in 88,707 ms |
| 2/3 | **Unraid `tower`** | **32** | 0 | 0 | PASS in 89,550 ms |
| 3/3 | **Unraid `tower`** | **32** | 0 | 0 | PASS in 88,521 ms |

> **RUN 3/3, exit 0**, 2026-08-09T18:44:40−05:00 → 18:49:07−05:00 (267 s wall), via
...
> | | run 1 | run 2 | run 3 |
> |---|---|---|---|
> | faulted reads to open the breaker | 40MiB **972 ms/3 req**, 44MiB **521 ms/1 req**, 48MiB **333 ms/0 req** | 40MiB 975 ms/3 req, 44MiB 592 ms/1 req, 48MiB 346 ms/0 req | 40MiB 989 ms/3 req, 44MiB 552 ms/1 req, 48MiB 360 ms/0 req |
>
...
runs were taken on **`235f2d3925e8c8f424516478b2f8312e0e4adcc6`**, and the commit that writes these figures
necessarily comes after them. Its diff against that tree is **documentation only** — `git diff --name-only
235f2d3..HEAD` lists exactly this file and the roadmap, and `deploy/`, `projectiond/`, `test/` and
...
- **THE HARNESS HAS SINCE COMPLETED ALL THREE ARMS** on the same frozen tree, and it **declares no winner**:
  it has no pass threshold and closes no G-number. `docs/PROJECTION_PHASE_2_RCLONE_BAKEOFF.md` §4 is the
  authority on its figures and on what they refuse to claim.
- **MEASUREMENT MODE DID NOT CARRY THE HARNESS RUN.** It was needed to get past a 1-sample overlap produced by
  a run in which the daemon could not read a single remote byte; with those defects fixed, arm A measured 6
  samples / 2.5 s, which clears the strict floor on its own. Said here because the opposite would be the
  natural assumption.
