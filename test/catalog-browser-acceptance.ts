...
test('the orchestrator covers the whole consumer workflow, in order, with a proof for each claim', () => {
  const required: Array<[RegExp, string]> = [
    ...
    [/acquisition or media-location namespace/,
      'and that the shipped runtime refuses an export carrying acquisition data'],
    [/the refusal echoed the hostile attribute key back to the terminal/,
      'and that the refusal names a position rather than the document-supplied key that caused it'],
    ...
    [/the producer's report echoed content it must not/,
      'while the production report itself names neither content nor the external system it came from'],
