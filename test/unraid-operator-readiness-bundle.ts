...
test('source and docs preserve offline planning boundary', () => {
  ...
  assert((scripts.test ?? '').includes('test/o5-kek-final-authorization.ts && tsx test/o5-disposition.ts && tsx test/launch-readiness-pass.ts && tsx test/launch-package.ts && tsx test/launch-candidate-dry-run.ts && tsx test/unraid-operator-readiness-bundle.ts'), 'aggregate order');
