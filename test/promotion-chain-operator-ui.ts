...
const RUNTIME_SECRETS = ['postgres_password', 'admin_database_url', 'database_url',
  // Phase 282. `custodian_root_key` seals the sidecar-managed KEK ring. It is declared by the stack from the
  // moment the ring exists as a capability — not from the moment a particular installation migrates onto it,
  // because a list that depended on which state an installation was in would be a list nobody could check.
  'completion_secret', 'custodian_kek', 'custodian_root_key', 'operator_ui_token'] as const;

/**
 * The one secret whose creation is a CUSTODY step rather than a generation step.
 *
 * PHASE 329. WHAT THE STACK DECLARES AND WHAT A GIVEN HOST CAN ESTABLISH ARE TWO DIFFERENT LISTS, and this
 * suite used to run them together. The root wrapping key must end up owned by the sidecar's runtime user and
 * readable by nobody else; only a POSIX host can produce that, and only `deploy/write-custody-secret.mjs` —
 * working on a descriptor, refusing rather than repairing — produces it there.
 *
 * The Bash script already behaved that way, and the Windows arm of this very test already asserted the
 * refusal: run `local-runtime-setup.sh` on a host with no ownership model and it exits non-zero, says
 * NOTHING WAS CREATED, and leaves no root key. The PowerShell script meanwhile wrote one through the ordinary
 * secret writer with a best-effort ACL, and this list — one list for both scripts — is what made that look
 * correct. Phase 329 makes the PowerShell script refuse the same secret for the same reason, so the list has
 * to split: what every host generates, and what only a custody-capable host can establish.
 */
const CUSTODY_SECRET = 'custodian_root_key';
const GENERATED_SECRETS = RUNTIME_SECRETS.filter((name) => name !== CUSTODY_SECRET);
...
function assertBootstrapped(
  label: string, ws: string, stdout: string, secrets: readonly string[] = RUNTIME_SECRETS,
...
for (const [label, scriptName, resolve] of [
  ...
  test(`${label} bootstraps a runnable stack, keeps every existing secret, and starts nothing`, () => {
    ...
    try {
      ...
      // PHASE 329. WHICH SECRETS THIS SCRIPT MAY BE HELD TO, ON THIS HOST. The PowerShell script refuses the
      // root wrapping key on every host it can run on, because Windows cannot give a file to the sidecar's
      // runtime user or withhold it from anybody else. The Bash script establishes it wherever it gets past
      // the refusal above, which by definition is a host that can.
      const custodyCapable = !scriptName.endsWith('.ps1');
      const expected = custodyCapable ? RUNTIME_SECRETS : GENERATED_SECRETS;
      const first = assertBootstrapped(label, ws, runSetup(shell, ws, scriptName), expected);

      if (!custodyCapable) {
        // THE REFUSAL IS THE ASSERTION, AND IT IS THE SAME OUTCOME THE BASH SCRIPT PRODUCES ON THIS HOST.
        // Before Phase 329 this script wrote a real 32-byte root wrapping key here through the ordinary
        // secret writer, with a best-effort ACL whose failure was swallowed — an unprotected key that every
        // custody check in this project would then read as custody established.
        assertEq(existsSync(join(ws, 'secrets', CUSTODY_SECRET)), false,
          `${label}: leaves no root wrapping key it cannot prove the ownership of`);
        assert(first.custodian_kek !== undefined, `${label}: while static KEK custody, which this stack uses, is set up`);
      }
      ...
      if (!custodyCapable) {
        // AND A RE-RUN STILL DOES NOT CREATE ONE. A setup script that refused once and quietly relented on the
        // second run would be worse than one that never refused, because nobody re-reads the second run.
        assertEq(existsSync(join(ws, 'secrets', CUSTODY_SECRET)), false,
          `${label}: a re-run does not create the root wrapping key either`);
        assert(second.includes('NOTHING WAS CREATED'), `${label}: and says so again, in the same words`);
      }
