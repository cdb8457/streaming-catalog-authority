...
const RUNTIME_SECRETS = ['postgres_password', 'admin_database_url', 'database_url',
  // Phase 282. `custodian_root_key` seals the sidecar-managed KEK ring. It is generated by the setup scripts
  // and declared by the stack from the moment the ring exists as a capability — not from the moment a
  // particular installation migrates onto it, because a list that depended on which state an installation was
  // in would be a list nobody could check.
