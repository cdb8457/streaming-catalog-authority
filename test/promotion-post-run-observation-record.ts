...
await test('REGRESSION: an impossible calendar observation time is rejected, never silently normalised', () => {
  const root = workspace();
  try {
    const auth = approvedAuthorizationFor(root);
    ...
    for (const stamp of IMPOSSIBLE_MOMENTS) {
      const o = buildPostRunObservationRecord({ authorizationRecord: auth, observation: completedObservation(auth, { observedAtUtc: stamp }) });
      assertEq(o.overall, 'POST_RUN_OBSERVATION_INVALID', `rejected: ${stamp}`);
      assert(o.blockers.includes('OBSERVATION_OBSERVED_AT_REQUIRED'), `${stamp} -> OBSERVATION_OBSERVED_AT_REQUIRED`);
      assertEq(o.observationRecorded, false, `nothing recorded: ${stamp}`);
    }
    for (const stamp of REAL_MOMENTS) {
      const o = buildPostRunObservationRecord({ authorizationRecord: auth, observation: completedObservation(auth, { observedAtUtc: stamp }) });
      assert(!o.blockers.includes('OBSERVATION_OBSERVED_AT_REQUIRED'), `a real moment must still be accepted: ${stamp}`);
    }
  } finally { rmSync(root, { recursive: true, force: true }); }
});
