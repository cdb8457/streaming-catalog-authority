...
test('jellyfin smoke ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Phase 13 validation: doc + suite wired; globalThis.fetch limited to operator CLIs', () => {
  ...
  const withFetch = walkTs(fileURLToPath(new URL('../src', import.meta.url)))
    ...
    .map((f) => f.replace(/\\/g, '/'));
  assert(withFetch.length === 6, `exactly six src files use globalThis.fetch (got: ${withFetch.join(', ')})`);
  assert(withFetch.every((f) => /src\/ops\/(jellyfin-smoke-cli|jellyfin-live-readonly-smoke-cli|jellyfin-live-evidence-capture-cli|jellyfin-live-readonly-mapping-cli|publish-reconcile-cli|torbox-smoke-cli)\.ts$/.test(f)), 'globalThis.fetch only in the operator smoke/reconcile CLIs');
});
...
test('Phase 219 Jellyfin live read-only mapping captures boundary evidence', () => {
  ...
  assert(
    (pkg.scripts.test ?? '').includes('test/jellyfin-smoke.ts && tsx test/jellyfin-live-readonly-mapping.ts && tsx test/deploy.ts'),
