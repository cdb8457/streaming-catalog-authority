...
test('Phase 147 operator UI service is long-running, read-only, and intentionally exposed', () => {
  ...
  assert(
    unraidCompose.includes('"8099:8099"') && unraidRuntimeCompose.includes('"${OPERATOR_UI_BIND_ADDRESS:-127.0.0.1}:${OPERATOR_UI_HOST_PORT:-8099}:8099"'),
...
test('Phase 217 scheduled doctor alert fix documents canonical launcher wrapper', () => {
  assert(exists('docs/PHASE_217_SCHEDULED_DOCTOR_ALERT_FIX.md'), 'Phase 217 scheduled doctor fix doc exists');
  ...
  const combined = [
    read('docs/PHASE_217_SCHEDULED_DOCTOR_ALERT_FIX.md'),
    read('docs/PHASE_20_UNRAID_OPERATIONS_SCHEDULE.md'),
    read('test/scheduled-doctor-alert-fix.ts'),
    read('README.md'),
  ].join('\n');
  for (const required of [
    ...
    'O4 remains `O4_CLOSED`',
    'O5 remains `O5_DEFERRED_ACCEPTED`',
  ]) assert(combined.includes(required), `Phase 217 surface preserves ${required}`);
  ...
  for (const forbidden of [
    ...
    'postgres://',
    'postgresql://',
  ]) assert(!doc.includes(forbidden), `Phase 217 record excludes ${forbidden}`);
});

test('Phase 218 Jellyfin live read-only evidence acceptance records retained smoke proof', () => {
  assert(exists('docs/PHASE_218_JELLYFIN_LIVE_READONLY_EVIDENCE_ACCEPTANCE.md'), 'Phase 218 Jellyfin acceptance doc exists');
  ...
  const combined = [
    read('docs/PHASE_218_JELLYFIN_LIVE_READONLY_EVIDENCE_ACCEPTANCE.md'),
    read('test/jellyfin-live-readonly-evidence-acceptance.ts'),
    read('README.md'),
  ].join('\n');
  for (const required of [
    ...
    'O4 remains `O4_CLOSED`',
    'O5 remains `O5_DEFERRED_ACCEPTED`',
  ]) assert(combined.includes(required), `Phase 218 surface preserves ${required}`);
  ...
  for (const forbidden of [
    ...
    'postgresql://',
    'tt0133093',
  ]) assert(!doc.includes(forbidden), `Phase 218 record excludes ${forbidden}`);
});

test('Phase 219 Jellyfin live read-only mapping captures boundary evidence', () => {
  assert(exists('docs/PHASE_219_JELLYFIN_LIVE_READONLY_MAPPING.md'), 'Phase 219 Jellyfin mapping doc exists');
  assert(exists('src/ops/jellyfin-live-readonly-mapping.ts'), 'Phase 219 Jellyfin mapping module exists');
  assert(exists('src/ops/jellyfin-live-readonly-mapping-cli.ts'), 'Phase 219 Jellyfin mapping CLI exists');
  assert(exists('deploy/unraid-jellyfin-live-mapping-capture.sh'), 'Phase 219 Jellyfin mapping launcher exists');
  assert(exists('test/jellyfin-live-readonly-mapping.ts'), 'Phase 219 Jellyfin mapping test exists');
  ...
  for (const required of [
    'phase-219-jellyfin-live-readonly-mapping',
    ...
    'JELLYFIN_LIVE_READONLY_MAPPING_BOUNDARY_ACCEPTED_NO_ELIGIBLE_ITEMS',
    'dataPositiveMappingEvidence: false',
    'does not launch runtime Jellyfin integration',
    'O4 remains `O4_CLOSED`',
    'O5 remains `O5_DEFERRED_ACCEPTED`',
  ]) assert(combined.includes(required), `Phase 219 surface preserves ${required}`);
  const doc = read('docs/PHASE_219_JELLYFIN_LIVE_READONLY_MAPPING.md');
  for (const forbidden of [
    'O5_CLOSED',
    'JELLYFIN_INTEGRATION_LAUNCHED',
    'provider mode enabled',
...
test('Phase 221 Jellyfin write-capable disposable collection proof is guarded and self-cleaning', () => {
  assert(exists('docs/PHASE_221_JELLYFIN_WRITE_PROOF.md'), 'Phase 221 Jellyfin write proof doc exists');
  assert(exists('src/ops/jellyfin-write-proof.ts'), 'Phase 221 Jellyfin write proof module exists');
  assert(exists('src/ops/jellyfin-write-proof-cli.ts'), 'Phase 221 Jellyfin write proof CLI exists');
  assert(exists('deploy/unraid-jellyfin-write-proof.sh'), 'Phase 221 Jellyfin write proof launcher exists');
  assert(exists('test/jellyfin-write-proof.ts'), 'Phase 221 Jellyfin write proof test exists');
  ...
  const combined = [
    read('docs/PHASE_221_JELLYFIN_WRITE_PROOF.md'),
    read('src/ops/jellyfin-write-proof.ts'),
    read('src/ops/jellyfin-write-proof-cli.ts'),
    read('deploy/unraid-jellyfin-write-proof.sh'),
    read('test/jellyfin-write-proof.ts'),
    read('README.md'),
  ].join('\n');
  for (const required of [
    ...
    'O4 remains `O4_CLOSED`',
    'O5 remains `O5_DEFERRED_ACCEPTED`',
  ]) assert(combined.includes(required), `Phase 221 surface preserves ${required}`);
...
test('Phase 226 Jellyfin test library preflight blocks Gelato and missing mount', () => {
  ...
  for (const required of [
    'phase-226-jellyfin-test-library-preflight',
    'Catalog Authority Test',
    '/media/catalog-authority-test-library',
    '/mnt/user/media/catalog-authority-test-library',
    'container-mount',
    'virtual-folder',
    'not-gelato',
...
test('Phase 226 local media E2E launcher is isolated and read-only against Jellyfin', () => {
  ...
  for (const required of [
    ...
    '$APPDATA_DIR/sidecar/run:/run/catalog-sidecar',
...
test('Phase 230 guarded promotion implementation is approval-gated and reversible', () => {
  assert(exists('src/ops/real-library-promotion.ts'), 'Phase 230 promotion service module exists');
  assert(exists('src/ops/real-library-promotion-cli.ts'), 'Phase 230 promotion CLI exists');
  assert(exists('deploy/unraid-real-library-promotion.sh'), 'Phase 230 Unraid promotion launcher exists');
  assert(exists('test/real-library-promotion.ts'), 'Phase 230 promotion test exists');
  ...
  const combined = [
    read('src/ops/real-library-promotion.ts'),
    read('src/ops/real-library-promotion-cli.ts'),
    read('deploy/unraid-real-library-promotion.sh'),
    read('test/real-library-promotion.ts'),
  ].join('\n');
  for (const required of [
    'phase-230-real-library-promotion',
    'PROMOTION_APPROVED=true',
    'PROMOTION_APPROVAL_REQUIRED',
    ...
    '/mnt/user/media/Movies',
    'refusing Gelato/AIO promotion path',
    'JELLYFIN_ALLOW_LIVE_PUBLISH=false',
    '--withdraw-after',
    'returnedToBefore',
    'absentAfterWithdrawal',
    'overwrite-real-library-file',
  ]) assert(combined.includes(required), `Phase 230 surface preserves ${required}`);
  for (const forbidden of [
    'JELLYFIN_ALLOW_LIVE_PUBLISH=true',
    'provider live mode enabled',
    'download enabled',
