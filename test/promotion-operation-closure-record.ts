...
await test('REGRESSION: an impossible calendar closing time is rejected, never silently normalised', () => {
  const root = workspace();
  try {
    const disp = acceptedDispositionFor(root);
    ...
    for (const stamp of IMPOSSIBLE_MOMENTS) {
      const c = buildOperationClosureRecord({ dispositionRecord: disp, closure: closedRecord(disp, { closedAtUtc: stamp }) });
      assertEq(c.overall, 'OPERATION_CLOSURE_INVALID', `rejected: ${stamp}`);
      assert(c.blockers.includes('CLOSURE_CLOSED_AT_REQUIRED'), `${stamp} -> CLOSURE_CLOSED_AT_REQUIRED`);
      assertEq(c.operationClosed, false, `nothing closed: ${stamp}`);
    }
    for (const stamp of REAL_MOMENTS) {
      const c = buildOperationClosureRecord({ dispositionRecord: disp, closure: closedRecord(disp, { closedAtUtc: stamp }) });
      assert(!c.blockers.includes('CLOSURE_CLOSED_AT_REQUIRED'), `a real moment must still be accepted: ${stamp}`);
    }
  } finally { rmSync(root, { recursive: true, force: true }); }
});
