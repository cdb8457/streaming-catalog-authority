...
await test('package, deploy guard, and README wire Phase 213 verification', () => {
  ...
  assert((pkg.scripts.test ?? '').includes('test/jellyfin-secret-readiness.ts && tsx test/jellyfin-container-command-shape.ts && tsx test/unraid-operator-readiness-bundle.ts'), 'aggregate order present');
