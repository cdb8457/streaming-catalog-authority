...
await test('package, deploy guard, and README wire Phase 206 verification', () => {
  ...
  assert((pkg.scripts.test ?? '').includes('test/jellyfin-readonly-mapping.ts && tsx test/jellyfin-disposable-write.ts && tsx test/jellyfin-evidence-review-decision.ts && tsx test/jellyfin-live-evidence-preflight.ts && tsx test/jellyfin-live-readonly-smoke-runner.ts && tsx test/unraid-operator-readiness-bundle.ts'), 'aggregate order present');
