...
import { runRetention, type RetentionFailpoint } from '../../src/ops/backup-retention.js';
import type { RetentionPolicy } from '../../src/ops/retention-model.js';

// Phase 310 — a prune that really stops existing, at a named boundary.
//
// -----------------------------------------------------------------------------------------------------
...
//
// So this runs the REAL retention in its own process and calls `process.exit` at the boundary. Everything in
...
function main(): number {
  ...
  try {
    runRetention(
      ...
      config.policy,
      { now: () => new Date(config.nowMs), suffix: () => config.suffix, at },
      { kind: 'run', confirm: config.confirm },
    );
  } catch (err) {
    // The run refused for a reason of its own before it reached the boundary. That is a different outcome
