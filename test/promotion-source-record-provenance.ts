...
await test('REGRESSION: an impossible calendar commitment time is rejected, never silently normalised', () => {
  const root = workspace();
  try {
    const { replay, chain } = verifiedReplay(root);
    ...
    for (const stamp of IMPOSSIBLE_MOMENTS) {
      const p = buildProvenanceCommitment({ replay, manifest: committedManifest(replay, chain, { committedAtUtc: stamp }) });
      assertEq(p.overall, 'PROVENANCE_INVALID', `rejected: ${stamp}`);
      assert(p.blockers.includes('MANIFEST_COMMITTED_AT_REQUIRED'), `${stamp} -> MANIFEST_COMMITTED_AT_REQUIRED`);
      assertEq(p.provenanceCommitted, false, `nothing committed: ${stamp}`);
    }
    for (const stamp of REAL_MOMENTS) {
      const p = buildProvenanceCommitment({ replay, manifest: committedManifest(replay, chain, { committedAtUtc: stamp }) });
      assert(!p.blockers.includes('MANIFEST_COMMITTED_AT_REQUIRED'), `a real moment must still be accepted: ${stamp}`);
    }
  } finally { rmSync(root, { recursive: true, force: true }); }
});
