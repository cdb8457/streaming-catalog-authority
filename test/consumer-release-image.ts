...
test('publishing is gated to a release or a deliberate dispatch from a version tag', () => {
  ...
  assertEq(stringList(publish.needs ?? null, 'needs').sort().join(','), 'bundle,image,release-candidate,suites',
    'nothing publishes before the image, the bundle, and the browser acceptance have all been checked');
