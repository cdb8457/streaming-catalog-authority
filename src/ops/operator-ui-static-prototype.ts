...
function renderOverviewCards(packets: readonly OperatorUiFixturePacket[]): string {
  return packets.slice(0, 4).map((packet) => {
    ...
    return [
      ...
      `<div class="summary-label">${escapeHtml(packet.screenLabel)}</div>`,
      `<h3>${escapeHtml(SCREEN_TITLES[packet.screenId])}</h3>`,
      `<div class="summary-state">${first === undefined ? 'Static' : renderStatus(first)}</div>`,
      '</article>',
    ].join('');
  }).join('');
}
...
function renderPacketPanel(packet: OperatorUiFixturePacket): string {
  ...
  return [
    ...
    '</div>',
    '<span class="panel-mode">Read Only</span>',
    '</div>',
    '<table>',
...
export function renderOperatorUiStaticPrototypeHtml(): string {
  ...
  return [
    ...
    '.sidebar{position:sticky;top:0;height:100vh;background:#1A1B1E;border-right:1px solid #303238;padding:18px 14px;}',
    ...
    '.brand p,.panel-heading p,.summary-label{margin:0;color:#A0A3A8;font-size:12px;text-transform:uppercase;letter-spacing:.08em;}',
    '.nav-link{display:block;color:#ECECEC;text-decoration:none;border:1px solid transparent;border-radius:6px;padding:9px 10px;margin:3px 0;font-size:13px;}',
    '.nav-link:first-of-type{border-color:#D18A3A;background:#232428;}',
    '.content{min-width:0;}',
    '.top-strip{position:sticky;top:0;z-index:1;background:#1A1B1E;border-bottom:1px solid #303238;display:flex;gap:10px;align-items:center;justify-content:space-between;padding:12px 18px;}',
    '.strip-group{display:flex;flex-wrap:wrap;gap:8px;align-items:center;}',
    '.strip-pill,.panel-mode{border:1px solid #303238;border-radius:4px;background:#232428;color:#A0A3A8;padding:5px 8px;font-size:12px;}',
    'main{padding:18px;display:grid;gap:16px;}',
    '.summary-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;}',
    '.summary-card,.screen-panel,.activity-panel{background:#1A1B1E;border:1px solid #303238;border-radius:8px;}',
    '.summary-card{padding:14px;min-height:112px;}',
    '.summary-card h3{font-size:17px;margin:8px 0 14px;letter-spacing:0;}',
    ...
    '@media (max-width:900px){.shell{grid-template-columns:1fr}.sidebar{position:relative;height:auto}.summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.top-strip{position:relative;align-items:flex-start;flex-direction:column}}',
    '@media (max-width:560px){main{padding:12px}.summary-grid{grid-template-columns:1fr}th:nth-child(4),td:nth-child(4){display:none}}',
    '</style>',
    '</head>',
    '<body>',
    '<div class="shell">',
    ...
    '<div class="strip-group"><strong>Operator UI Static Prototype</strong><span class="strip-pill">Fixture Only</span><span class="strip-pill">Read Only</span></div>',
    '<div class="strip-group"><span class="status-chip status-info">Provider Count</span><span class="status-chip status-warning">O4 O5 Deferred</span></div>',
    '</header>',
    '<main>',
    `<section class="summary-grid" aria-label="Overview status">${renderOverviewCards(packets)}</section>`,
    '<section class="activity-panel"><h2>Sanitized Activity</h2>',
