...
export const TORBOX_LIVE_SMOKE_PLAN: TorBoxLiveSmokePlan = {
  ...
  steps: [
    ...
    },
    {
      ...
      operatorAction: 'Run manually outside CI and retain only the redacted JSON report. Use --silent and the extra -- separator so npm does not consume smoke flags or add headers to JSON.',
    },
    {
      number: 3,
    ...
    },
    {
      ...
      status: 'static-only',
      commandShapes: [
        'npm run --silent ops:torbox-live-smoke-evidence-preflight -- -- <phase-43-service-status-report.json> --json',
        'npm run --silent ops:torbox-live-smoke-evidence-preflight -- -- <phase-43-hoster-metadata-report.json> --json',
        'npm run --silent ops:torbox-live-smoke-evidence-preflight -- -- <phase-43-cache-report.json> --json',
      ],
