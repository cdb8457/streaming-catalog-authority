// A small YAML reader for the Compose files and CI workflows in this repository, so tests AND the
// release-readiness proof (Phase 250) can assert what a stack or a pipeline IS rather than how its file
// happens to be typed. It lives in src/ so production code (the readiness CLI) can share the one
// implementation that the test suites already exercise; test/helpers/compose-yaml.ts re-exports it.
//
