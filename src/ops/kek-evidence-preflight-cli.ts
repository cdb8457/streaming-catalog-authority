...
type DescriptorFileReadResult =
  ...
  | { readonly ok: false; readonly code: KekEvidencePreflightInputErrorCode };

/**
 * Phase 30 - KEK custody and scheduling evidence descriptor preflight.
 *
 *   tsx src/ops/kek-evidence-preflight-cli.ts <descriptor.json> [--json]
 *   (or: npm run ops:kek-evidence-preflight -- -- <descriptor.json> --json)
 *
 * LOCAL DESCRIPTOR PREFLIGHT ONLY. Reads exactly one operator-supplied JSON descriptor file, then
 * validates metadata for O5 evidence review. It does not read env values, scan directories, connect
 * to a database, call remote APIs, run container tooling, invoke age, inspect key files, rotate keys,
 * contact custody services, install scheduling, or close O5.
 */
function main(): number {
  const args = process.argv.slice(2);
  if (args.includes('--help') || args.includes('-h')) {
    console.log('usage: ops:kek-evidence-preflight -- <descriptor.json> [--json]');
    return 0;
  }
  ...
  if (unsupported.length > 0 || paths.length !== 1) {
    console.error('usage: ops:kek-evidence-preflight -- <descriptor.json> [--json]');
    return 2;
  }
  ...
  const report = input.ok
    ...
    : buildKekEvidencePreflightInputErrorReport(input.code);

  process.stdout.write(asJson ? formatKekEvidencePreflightJson(report) : formatKekEvidencePreflightText(report));
  return reportHasFailures(report) ? 1 : 0;
}

function buildReportFromJson(jsonText: string) {
  const parsed = parseKekEvidenceDescriptorJson(jsonText);
  if (typeof parsed === 'string') return buildKekEvidencePreflightInputErrorReport(parsed);
  return buildKekEvidencePreflightReport(parsed);
}
