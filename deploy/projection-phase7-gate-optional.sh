#!/usr/bin/env bash
# The Phase 7 gate on a host where it is OPTIONAL: 77 becomes 0, and nothing else changes.
#
...
GATE_SKIP_STATUS=77
GATE_COMMAND="${PROJECTION_RELIABILITY_GATE_COMMAND:-$HERE/projection-phase7-gate.sh}"
...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "NOTHING WAS PROVED. The Phase 7 gate skipped on this host and this entry point maps that skip" >&2
