#!/usr/bin/env bash
# THREE CONSECUTIVE FRESH RUNS OF THE PHASE 7 GATE - the command Phase 7 closes on.
#
...
RUNS="${PROJECTION_PHASE7_GATE_RUNS:-3}"
...
GATE_COMMAND="${PROJECTION_PHASE7_GATE_COMMAND:-$HERE/projection-phase7-gate.sh}"

completed=0
for run in $(seq 1 "$RUNS"); do
  echo
  echo "############################################################"
  echo "# Projection Phase 7: run $run of $RUNS"
  echo "############################################################"
  bash "$GATE_COMMAND"
  status=$?
...
# `PROJECTION_PHASE7_GATE_RUNS=0`, say — must not be able to announce a completed sequence either.
