...
RUNS="${PROJECTION_LIFECYCLE_GATE_RUNS:-3}"
...
for run in $(seq 1 "$RUNS"); do
  echo
  echo "############################################################"
  echo "# path lifecycle gate: run $run of $RUNS"
  echo "############################################################"
  bash "$GATE_COMMAND"
  status=$?
...
# `PROJECTION_LIFECYCLE_GATE_RUNS=0`, say -- must not be able to announce a completed sequence either.
...
echo
echo "$completed of $RUNS consecutive path-lifecycle runs completed, none skipped. On a Linux or Unraid host"
