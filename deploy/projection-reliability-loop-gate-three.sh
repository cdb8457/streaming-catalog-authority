#!/usr/bin/env bash
# THREE CONSECUTIVE FRESH RUNS OF THE PHASE 3 RELIABILITY LOOP — the command Phase 3 closes on.
#
...
RUNS="${PROJECTION_RELIABILITY_GATE_RUNS:-3}"
...
for run in $(seq 1 "$RUNS"); do
  echo
  echo "############################################################"
  echo "# Projection Phase 3 reliability loop: run $run of $RUNS"
  echo "############################################################"
  bash "$GATE_COMMAND"
  status=$?
...
# `PROJECTION_RELIABILITY_GATE_RUNS=0`, say — must not be able to announce a completed sequence either.
...
echo
echo "$completed of $RUNS consecutive reliability-loop runs completed, none skipped."
echo
...
echo "  provider and then broke it six times on purpose — a graceful restart, a SIGKILL, a mount taken out"
echo "  from under a living daemon, a provider outage past the breaker's cooldown, a credential rotation and"
echo "  a restart of all three frontends — checking the operator's own approved windows through the mount AND"
echo "  inside each server's container before and after every one of them."
echo
