#!/usr/bin/env bash
# The REAL TORBOX acceptance gate for a host where it is OPTIONAL.
#
...
# `projection-torbox-real-gate.sh` and maps its SKIP status (77) to 0, so such a job stays green when
...
#
# WHAT ITS SKIP CONDITIONS ARE. TWO, and the second is the important one. /dev/fuse unreachable from a
# container; and an
# operator who has supplied no TorBox corpus, in which case NOTHING WAS CONTACTED and no provider assertion
# was evaluated. A 77 here is emphatically not evidence about a provider -- it is the absence of any.
#
# WHAT IT IS NOT FOR, AND THE DISTINCTION IS THE POINT. It is NOT the command an acceptance plan names as
# evidence. `npm run go:torbox-real-gate` and `go:torbox-real-gate:three` propagate 77, so a required
# acceptance
...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "OPTIONAL HOST: the real TorBox acceptance gate skipped (status ${GATE_SKIP_STATUS}) and this" >&2
  echo "entry point treats that as success. NOTHING WAS PROVED. No acceptance gate is closed by this run." >&2
  echo "Use 'npm run go:torbox-real-gate:three' on a host that can host it for evidence." >&2
  exit 0
fi
exit "$status"
