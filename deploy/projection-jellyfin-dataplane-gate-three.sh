...
GATE_COMMAND="${PROJECTION_JELLYFIN_GATE_COMMAND:-$HERE/projection-jellyfin-dataplane-gate.sh}"
...
# `PROJECTION_JELLYFIN_GATE_RUNS=0`, say -- must not be able to announce a completed sequence either.
...
echo
echo "$completed of $RUNS consecutive runs completed, none skipped. On a Linux or Unraid host this is the"
echo "repetition the acceptance plan asks for; on Windows or Docker Desktop it is not, and must not be"
echo "reported as Phase 1 closure."
