#!/usr/bin/env bash
# The mount-truth gate for a host where it is OPTIONAL.
#
...
# `npm run go:mount-truth-gate` propagates 77, so a required acceptance invocation on a host that cannot host
...
GATE_SKIP_STATUS=77
GATE_COMMAND="${PROJECTION_MOUNT_TRUTH_GATE_COMMAND:-$HERE/projection-mount-truth-gate.sh}"
...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "OPTIONAL HOST: the mount-truth gate skipped (status ${GATE_SKIP_STATUS}) and this entry point treats" >&2
  echo "that as success. NOTHING WAS PROVED. Phase 4 is not closed by this run. Use" >&2
  echo "'npm run go:mount-truth-gate' on a host that can host it for evidence." >&2
  exit 0
fi
exit "$status"
