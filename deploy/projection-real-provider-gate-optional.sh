#!/usr/bin/env bash
# The REAL PROVIDER correctness gate for a host where it is OPTIONAL.
#
...
# `projection-real-provider-gate.sh` and maps its SKIP status (77) to 0, so such a job stays green when
...
#
# WHAT ITS SKIP CONDITIONS ARE. TWO. /dev/fuse unreachable from a container, and -- in real mode -- an
# operator who has
# supplied no corpus. NEITHER IS A PASS: the second in particular means nothing was contacted and no
# assertion was evaluated. To exercise every assertion offline instead, run `npm run
# go:real-provider-gate:fake`,
# which is a real run of the gate rather than a skip of it.
#
# WHAT IT IS NOT FOR, AND THE DISTINCTION IS THE POINT. It is NOT the command an acceptance plan names as
# evidence. `npm run go:real-provider-gate` and `go:real-provider-gate:three` propagate 77, so a required
# acceptance
...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "OPTIONAL HOST: the real-provider correctness gate skipped (status ${GATE_SKIP_STATUS}) and this" >&2
  echo "entry point treats that as success. NOTHING WAS PROVED. No acceptance gate is closed by this run." >&2
  echo "Use 'npm run go:real-provider-gate:three' on a host that can host it for evidence." >&2
  exit 0
fi
exit "$status"
