...
# The Phase 251 verification packet ships as a third asset when release assembly produced one: a consumer
# downloads it next to the archive and verifies the download offline. It is optional here so this script still
# runs on a machine that only has the archive and its checksum (the tests exercise exactly that path).
PACKET="${ARCHIVE}.verification.json"
ASSETS=("${ARCHIVE}" "${ARCHIVE}.sha256")
if [ -f "${PACKET}" ]; then
  echo "==> verifying the release integrity packet describes this exact archive"
  # Run from the repository root (where tsx resolves) with absolute paths, in a subshell so the archive
  # directory stays the working directory for the `gh` steps that follow.
  ( cd "${REPO_ROOT}" && node --import tsx "${REPO_ROOT}/src/ops/release-verification-cli.ts" --verify \
      --archive "${ARCHIVE_DIR_ABS}/${ARCHIVE}" --packet "${ARCHIVE_DIR_ABS}/${PACKET}" )
  ASSETS+=("${PACKET}")
fi
