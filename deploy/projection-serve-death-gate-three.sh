#!/usr/bin/env bash
# The SERVE-DEATH GATE, three times: a runner that proves the gate is repeatable, not lucky.
#
# WHY THREE. A serve death is a kernel conversation — the mountpoint is taken out from under the daemon by an
# external unmount through a shared mount — and the same conversation on the same host is the same sequence
# every time, so the flake a single run cannot see is the one that only shows up when the conversation
# happens again. Three clean runs with every run asserted the same way is the difference between "it passed
# on a good day" and "it passes".
#
...
# runs: each run is a cold start of PostgreSQL and the daemon, exactly as `npm run go:serve-death-gate`
# would start it. The two runs a CI user actually cares about are the direct one and this one, and this one
# is deliberately the same gate again, not a trimmed variant.
set -euo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
GATE="$HERE/projection-serve-death-gate.sh"
...
if docker ps --filter "name=projection-serve-death-" --format '{{.Names}}' | grep -q .; then
  echo "REFUSING TO RUN: a previous serve-death container is still alive." >&2
...
for round in 1 2 3; do
  echo
  echo "==== serve-death gate, run $round/3 ===="
  set +e
  START="$(date +%s%3N)"
  bash "$GATE"
  RC=$?
...
echo
echo "serve-death gate: ${PASS} pass, ${FAIL} fail${TIMES}"
if [ "$FAIL" -ne 0 ]; then
