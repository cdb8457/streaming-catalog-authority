...
// AN EMPTY DESCRIPTOR IS NOT A DESCRIPTOR. `objects[0]` on an endpoint that registered nothing is undefined
// and the next line raised a TypeError -- fail-closed, but as a crash rather than as the statement it is.
if (!Array.isArray(objects) || objects.length === 0) {
  console.error('the range endpoint described no objects');
  process.exit(1);
}
...
# THE HEADING WAS THE WHOLE ASSERTION. These two lines printed a byte count and the script walked straight
# past it, so "returns bytes" was never checked against the number underneath it. `dd`'s own exit status is
# lost to the pipe and `dd` reports success for a read that lands past EOF anyway, so the count IS the
# measurement. Measured in the gate's own pinned image: a seek into a file that does not exist at all prints
# `0` with a pipeline status of 0, and this phase went on to report every mutation refused and pass.
seek_returns_bytes() {
  seek_count="$(dd if="$1" bs=1024 skip="$2" count=1 2>/dev/null | wc -c | tr -d ' ')"
  echo "    $1 at block $2 returned ${seek_count} byte(s)"
  # GREATER THAN ZERO IS THE STEP'S OWN CLAIM, and no more than that is asserted here: a FUSE read may
  # legitimately return a short block, and turning this into an exact-length check would invent a threshold
  # the gate never stated. What is refused is the zero.
  test "${seek_count:-0}" -gt 0 \
    || { echo "a seek into $1 returned no bytes at all" >&2; exit 1; }
}
seek_returns_bytes "/mnt/Movies/Local One/Local One.bin" 512
seek_returns_bytes "/mnt/Movies/Remote Two/Remote Two.bin" 2048
