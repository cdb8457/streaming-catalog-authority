#!/usr/bin/env bash
# The operational mount health gate for a host where it is OPTIONAL.
#
...
# `npm run go:mount-health-gate` propagates 77, so a required acceptance invocation on a host that cannot host
...
GATE_SKIP_STATUS=77
GATE_COMMAND="${PROJECTION_MOUNT_HEALTH_GATE_COMMAND:-$HERE/projection-operational-mount-health-gate.sh}"
...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "OPTIONAL HOST: the mount-health gate skipped (status ${GATE_SKIP_STATUS}) and this entry point treats" >&2
  echo "that as success. NOTHING WAS PROVED. Phase 5 is not closed by this run. Use" >&2
  echo "'npm run go:mount-health-gate' on a host that can host it for evidence." >&2
  exit 0
fi
exit "$status"
