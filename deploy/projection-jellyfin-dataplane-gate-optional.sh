...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "OPTIONAL HOST: the data-plane gate skipped (status ${GATE_SKIP_STATUS}) and this entry point treats" >&2
  echo "that as success. NOTHING WAS PROVED. No acceptance gate is closed by this run. Use" >&2
  echo "'npm run go:jellyfin-dataplane-gate:three' on a host that can host it for evidence." >&2
  exit 0
fi
exit "$status"
