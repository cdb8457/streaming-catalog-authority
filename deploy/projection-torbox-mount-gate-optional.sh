#!/usr/bin/env bash
# The TORBOX MOUNT gate for a host where it is OPTIONAL.
#
...
# `projection-torbox-mount-gate.sh` and maps its SKIP status (77) to 0, so such a job stays green when
...
# evidence. `npm run go:torbox-mount-gate` and `go:torbox-mount-gate:three` propagate 77, so a required
# acceptance
...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "OPTIONAL HOST: the TorBox mount gate skipped (status ${GATE_SKIP_STATUS}) and this" >&2
  echo "entry point treats that as success. NOTHING WAS PROVED. No acceptance gate is closed by this run." >&2
  echo "Use 'npm run go:torbox-mount-gate:three' on a host that can host it for evidence." >&2
  exit 0
fi
exit "$status"
