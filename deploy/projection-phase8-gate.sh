#!/usr/bin/env bash
# PROJECTION PHASE 8 - THE OPERATOR SOAK. Phase 7 proved the appliance survives SIX DELIBERATE FAULTS with
# three real, digest-pinned media servers attached and playing real provider bytes. This asks whether it
# survives BEING USED: the same cache, the same durable ledger, the same libraries, the same containers, the
# same binds and the same mount point, cycle after cycle, with NOTHING recreated between them.
#
# WHY IT EXISTS, IN ONE PARAGRAPH. Phase 7's own section 5 says every one of its runs is FRESH by design,
# "because the question is whether the appliance survives a fault FROM A CLEAN START". An operator never gets
# a clean start. Phase 7 section 12.4 ships five rough edges and three of them - a killed daemon's mount, a
# stop under a foreign overlay, and a rollback target that lives in the cache - are states an operator reaches
# by REPETITION rather than by injection. Nothing in this repository has ever run twice against the same
# cache, ledger, libraries and mount point.
#
# THE INVERSION IS THE WHOLE TRANCHE AND THIS SCRIPT IS BUILT AROUND IT. The SOAK is fresh; the CYCLES inside
# it are deliberately NOT, and a cycle that finds its cache, ledger, libraries or consumers new FAILS rather
# than quietly starting from a tidier place. `P8-cycle-inherited` is that assertion.
#
# THE CONTRACT AND ITS THRESHOLDS WERE WRITTEN DOWN BEFORE ANY OF THIS RAN.
# `docs/PROJECTION_PHASE_8_OPERATOR_SOAK.md` is the contract; `src/core/projection/phase8.ts` is the same
# thresholds as code, and THIS SCRIPT RESTATES NONE OF THEM - it evaluates them out of that module once, at
# the top, so a budget cannot drift between the document, the module and the shell. Three of them are new and
# every other one is IMPORTED from the closed tranche that owns it.
#
# THE SETUP IS PHASE 7's, DELIBERATELY UNCHANGED AND DELIBERATELY DUPLICATED. Every gate here is standalone;
# sharing a setup between two gates would mean a change made for one silently re-scoping the other. What is
# different begins at the cycle loop and nowhere before it.
#
# WHAT IT DOES NOT DO. It injects no SIGKILL - Phase 7 section 12.4 #1 already declares that limitation and
# `deploy/projection-restart-topology-gate.sh` RT4 already measures it, provider-free. It reboots nothing, it
# touches no unrelated container, network, volume or share, and it never writes the operator's endpoint file.
#
# A SKIP IS A FAILURE. There are no optional steps.
set -euo pipefail
...
COMPOSE_FILE="docker-compose.projection-phase8.yml"
NETWORK="projection-phase8-gate"
PG_PORT="${PROJECTION_PHASE8_GATE_PG_PORT:-5640}"
JF_PORT="${PROJECTION_PHASE8_GATE_JELLYFIN_PORT:-8290}"
EMBY_PORT="${PROJECTION_PHASE8_GATE_EMBY_PORT:-8291}"
RESOLVER_PORT="${PROJECTION_PHASE8_GATE_RESOLVER_PORT:-8292}"
...
REL_GATE_ROOT=".projection-phase8-gate"
REL=".projection-phase8-gate/run-$$"
WORK="$GATE_ROOT/run-$$"
...
P8_BUSYBOX_IMAGE="busybox@sha256:0872fb3a7632ba9d0ae46a8e832a62b30ce83a6f220b8bb52903d9cf477dabe3"
P8_HAS_NSENTER=0
command -v nsenter >/dev/null 2>&1 && P8_HAS_NSENTER=1
CLEANED=0
...
CYCLES_REL="$REL_GATE_ROOT/evidence/cycles-$$.jsonl"
...
export PROJECTION_PHASE8_GATE_PG_PORT="$PG_PORT"
...
P8_BUDGETS="$(npx tsx src/ops/projection-phase8-cli.ts budgets --sh)" \
  || die "the reliability-loop thresholds could not be read; nothing can be measured against nothing"
eval "$P8_BUDGETS"
npx tsx src/ops/projection-phase8-cli.ts budgets
...
# of a run are `P8-host-*-set-unchanged`, `P8-own-mountpoints-removed` and `P8-own-run-directory-removed`,
...
cat > "$WORK/out/cycle.cjs" <<'CYCLELOG'
// One cycle, appended as one JSON line. The closure check compares this SEQUENCE against the contract's
// cycle count and order, so three entries all naming cycle 1 cannot clear a count.
const { appendFileSync } = require('node:fs');
const [, , out, index] = process.argv;
const n = Number(index);
if (!Number.isSafeInteger(n) || n < 1) { console.error('cycle: not a cycle index'); process.exit(1); }
...
appendFileSync(out, JSON.stringify({ index: n, cycle: n }) + String.fromCharCode(10));
CYCLELOG
...
test "${WINDOW_COUNT:-0}" -ge "$P8_OPERATOR_WINDOWS_REQUIRED" \
  || die "the operator corpus carries $WINDOW_COUNT approved window(s), under the \
$P8_OPERATOR_WINDOWS_REQUIRED this contract needs to hold a byte-correctness claim"

node "$REL/out/config.cjs" "$REL/out/effective-endpoint.json" "$REL/config.json" \
  "$DAEMON_STATUS_PORT" "$P8_POLL_INTERVAL_MS" \
...
DAEMON_POLL="$(( P8_POLL_INTERVAL_MS / 1000 ))s"
test "$(( P8_POLL_INTERVAL_MS % 1000 ))" -eq 0 \
...
start_resolver() {
  ...
  RESOLVER_CONTAINER="projection-p8-resolver-$$-$RESOLVER_SEQ"
...
# to "recovered", judged `P8-F-A3-remounted-in-place` against a daemon that had not remounted yet, and read
...
# `P8-R-ready-ms` is required once per cycle, and two of the six arms never set this variable — so cycles 4
...
# not exist, `P8-R3-restart-left-no-layer`, which fails LOUDLY if the daemon's own stop leaves anything behind.
...
case "$resolver_reach" in
  0) record P8-A-resolver-loopback-only bool 0 "" "the resolver accepted a TCP connection from the gate network" || true
     die "the resolver is reachable from the gate network; it must be loopback-only" ;;
  1) record P8-A-resolver-loopback-only bool 1 "" "refused at the transport from the gate network" ;;
  *) record P8-A-resolver-loopback-only bool 0 "" "the reachability probe could not take the measurement" || true
...
if [ "$ffprobe_status" -eq 0 ] && grep -q "codec_type=video" "$WORK/out/ffprobe.txt"; then
  record P8-A-entry-is-decodable-video bool 1 "" "a decoder found a video stream in the projected entry"
  ...
  test "${REAL_DURATION_INT:-0}" -gt "$P8_TRANSCODE_DECODED_SECONDS_MIN" \
...
belongs here with a name rather than three phases later inside a media server that gets blamed for it"
  ...
  record P8-A-entry-is-decodable-video bool 0 "" "no video stream was found in the projected entry" || true
...
phase_bytes() {
  ...
  record "$win_id" ge "$matched" "$P8_OPERATOR_WINDOWS_REQUIRED" \
  ...
  if [ "$inread_prefix" != "-" ]; then
    ...
    for server in $P8_SERVERS; do
      set +e
      verdict="$(timeout "$(( ( P8_READ_FAIL_BUDGET_MS * 2 ) / 1000 ))" \
...
phase_churn() {
  ...
  for server in $P8_SERVERS; do
    ...
    set -e
    record "P8-cycle-churn:$server:$CYCLE_ID" le "$churn" "$P8_LIBRARY_CHURN_MAX" \
...
start_paced_play() {
  ...
  if [ "$server" = "emby" ]; then
    npx tsx "$(cli_for "$server")" paced-play \
      ...
      --key "$REAL_FILE" --seconds "$P8_PLAY_DECODED_SECONDS_MIN" \
...
play_all_three() {
  ...
  for server in $servers; do
    ...
    if [ "$id_prefix" = "P8-B" ]; then
      record "P8-S4-play-start-ms:$server:$CYCLE_ID" le "$startup" "$P8_PLAY_START_BUDGET_MS" \
        "from launching the consumer to its first decoded frame, with the other two also in flight" || true
      record "P8-S4-play-decoded-seconds:$server:$CYCLE_ID" ge "$decoded" "$P8_PLAY_DECODED_SECONDS_MIN" \
...
seeks_for() {
  ...
  if [ "$seek_status" -ne 0 ]; then
    ...
    record "P8-S4-seeks:$server:$CYCLE_ID" eq 0 "$P8_SEEK_COUNT" \
  ...
  record "P8-S4-seeks:$server:$CYCLE_ID" eq "${seeks:-0}" "$P8_SEEK_COUNT" \
...
transcode_for() {
  ...
  npx tsx "$(cli_for "$server")" transcode-soak --state "$(state_for "$server")" \
    ...
    --out "$REL/out/soak-$server.json" --seconds "$P8_TRANSCODE_DECODED_SECONDS_MIN" \
    > "$WORK/out/soak-log-$server.txt" 2>&1
  status=$?
  set -e
  if [ "$status" -ne 0 ]; then
    tail -25 "$WORK/out/soak-log-$server.txt" >&2 || true
    record "P8-S4-transcode-decoded-seconds:$server:$CYCLE_ID" ge 0 "$P8_TRANSCODE_DECODED_SECONDS_MIN" \
  ...
  if [ "$server" = "plex" ]; then
    npx tsx "$(cli_for "$server")" transcode-soak-verify --key "$REAL_FILE" \
      ...
      --seconds "$P8_TRANSCODE_DECODED_SECONDS_MIN" > "$WORK/out/soak-verify-$server.txt" 2>&1
  else
    npx tsx "$(cli_for "$server")" transcode-soak-verify --key "$REAL_FILE" \
      --items "$REL/out/items-$server.json" --soak "$REL/out/soak-$server.json" \
  ...
  if [ "$status" -ne 0 ]; then
    ...
    record "P8-S4-transcode-decoded-seconds:$server:$CYCLE_ID" ge "${decoded:-0}" \
      "$P8_TRANSCODE_DECODED_SECONDS_MIN" \
...
# ----------------------------------------------------------------------------------------------------------
# THE SHARED PER-CYCLE VERIFICATION — and a cycle that skips it proves nothing
# ----------------------------------------------------------------------------------------------------------
...
verify_after_cycle() {
  local layers rows
  phase_bytes "P8-cycle-windows:$CYCLE_ID" "P8-cycle-stat:$CYCLE_ID" "P8-cycle-seed:$CYCLE_ID" \
    "P8-cycle-inread" ":$CYCLE_ID" "cycle $CYCLE_ID"
  phase_catalogue P8-cycle-catalogue ":$CYCLE_ID" "cycle $CYCLE_ID"
  phase_churn ":$CYCLE_ID"
  ...
  if [ -n "$layers" ] && [ "$layers" -gt "$P8_MOUNT_LAYERS_ABOVE_FLOOR_MAX" ]; then
    echo "  --- mount survey: $layers of ours above a floor of $MOUNT_LAYER_FLOOR after cycle $CYCLE_ID ---" >&2
  ...
  record "P8-cycle-layers:$CYCLE_ID" le "$layers" "$P8_MOUNT_LAYERS_ABOVE_FLOOR_MAX" \
  ...
  bind_fingerprint "$WORK/out/binds-$CYCLE_ID.txt"
  if cmp -s "$WORK/out/binds-before.txt" "$WORK/out/binds-$CYCLE_ID.txt"; then
    record "P8-cycle-binds-unchanged:$CYCLE_ID" bool 1 "" \
      "the same three containers, started at the same instants, holding the same mounts as before the first mount"
  else
    record "P8-cycle-binds-unchanged:$CYCLE_ID" bool 0 "" \
      "a consumer container or one of its mounts is not the one that was attached before the first mount" || true
    diff "$WORK/out/binds-before.txt" "$WORK/out/binds-$CYCLE_ID.txt" >&2 || true
  fi
}
...
await_recovery_action() {
  ...
  started="$(date +%s%3N)"
  deadline=$(( started + P8_RECOVERY_ACTION_BUDGET_MS + 1000 ))
...
# ----------------------------------------------------------------------------------------------------------
# THE INHERITANCE CHECK — §5 of the contract, and it is the one assertion that makes this a soak
# ----------------------------------------------------------------------------------------------------------
# WHAT IT IS FOR. A cycle after the first must find the SAME cache, the SAME durable ledger, the SAME manifest
# generation, the SAME three media-server configuration directories, the SAME three container ids and a mount
# point that has been mounted before. If any of them is new, the run is three Phase 7 runs wearing a soak's
# name and every number below would mean something else entirely.
#
# IT IS IDENTITY AND NOT EXISTENCE. A cache directory that was deleted and recreated exists; it is not the one
# the previous cycle used. So each is fingerprinted the first time it is seen and compared afterwards, by
# inode and by the container's own start instant, which is what `bind_fingerprint` already does for the binds.
INHERIT_BASELINE="$WORK/out/inherit-before.txt"
inherit_fingerprint() {
  local out="$1" server
  : > "$out"
  # THE CACHE AND THE LEDGER, BY INODE. A recreated directory has a different one.
  stat -c 'cache %i' "$WORK/cache" >> "$out" 2>/dev/null || echo "cache UNREADABLE" >> "$out"
  if [ -e "$WORK/cache/recovery/recovery-ledger.json" ]; then
    stat -c 'ledger %i' "$WORK/cache/recovery/recovery-ledger.json" >> "$out" 2>/dev/null \
      || echo "ledger UNREADABLE" >> "$out"
  else
    echo "ledger ABSENT" >> "$out"
  fi
  stat -c 'manifest %i' "$WORK/manifest" >> "$out" 2>/dev/null || echo "manifest UNREADABLE" >> "$out"
  # THE THREE SERVERS, BY CONTAINER ID AND START INSTANT — the same two facts `bind_fingerprint` compares.
  for server in $P8_SERVERS; do
    local container
...
assert_inherited() {
  local now="$WORK/out/inherit-$CYCLE_ID.txt"
  if ! inherit_fingerprint "$now"; then
    record "P8-cycle-inherited:$CYCLE_ID" bool 0 "" \
      "something this cycle must inherit could not be read at all, so nothing about it was measured" || true
    return 0
  fi
  if [ "$CYCLE_INDEX" -eq 1 ]; then
    cp "$now" "$INHERIT_BASELINE"
    record "P8-cycle-inherited:$CYCLE_ID" bool 1 "" \
      "the first cycle establishes what every later one must find unchanged: $P8_INHERITED thing(s)"
    return 0
  fi
  if cmp -s "$INHERIT_BASELINE" "$now"; then
    record "P8-cycle-inherited:$CYCLE_ID" bool 1 "" \
      "the same cache, ledger, manifest, server configuration directories, container ids and mount point as \
cycle 1 — this cycle inherited rather than started over"
  else
    record "P8-cycle-inherited:$CYCLE_ID" bool 0 "" \
      "something this cycle should have inherited is a DIFFERENT object, so this is not a soak" || true
    diff "$INHERIT_BASELINE" "$now" >&2 || true
  fi
}

# ----------------------------------------------------------------------------------------------------------
# THE OPERATOR-INTERVENTION LEDGER — §4's `OPERATOR_INTERVENTIONS_MAX`
# ----------------------------------------------------------------------------------------------------------
# ANYTHING THIS GATE HAS TO DO BETWEEN TWO CYCLES FOR THE NEXT ONE TO WORK IS AN INTERVENTION, and it is
# counted rather than argued about. The gate calls `intervened` at the one place it could ever need to — the
# stale-mount remediation the shipped preflight prints — so a soak that needed a human is a soak that says so
# with a number instead of a paragraph.
OPERATOR_INTERVENTIONS=0
intervened() {
  OPERATOR_INTERVENTIONS=$(( OPERATOR_INTERVENTIONS + 1 ))
  echo "  OPERATOR INTERVENTION #$OPERATOR_INTERVENTIONS: $*" >&2
}
...
step_S1_preflight() {
  ...
  # gate here has ever asked it about.
  alpha preflight
  # AN HONEST PREFLIGHT IS ONE WHOSE VERDICT MATCHES THE MOUNT POINT. It may refuse — that is a legitimate
  # answer — but it must not refuse for a reason the mount point does not have, and it must not pass over a
  # dead mount. What is asserted is the pairing.
  local dead=0 refused=0
  if [ -e "$WORK/mnt" ] && ! ls "$WORK/mnt" >/dev/null 2>&1; then dead=1; fi
  if [ "$ALPHA_STATUS" -ne 0 ]; then refused=1; fi
  record "P8-S1-preflight-honest:$CYCLE_ID" bool \
    "$( [ "$dead" -eq "$refused" ] && echo 1 || echo 0 )" "" \
    "preflight exited $ALPHA_STATUS and the mount point is $( [ "$dead" -eq 1 ] && echo DEAD || echo readable ); \
an honest preflight refuses when and only when there is something to refuse" || true
  if [ "$dead" -eq 1 ]; then
    # THE SHIPPED REMEDIATION, AND TAKING IT IS AN INTERVENTION THAT COUNTS AGAINST THE THRESHOLD.
...
step_S2_install_start_status() {
  ...
  # trusted.
  local says_running reads
  says_running=0
  grep -q "container *running" "$WORK/out/alpha-status-$CYCLE_ID.txt" && says_running=1
  reads=0
  await_readable 120 && reads=1
  record "P8-S2-status-truthful:$CYCLE_ID" bool \
    "$( [ "$says_running" -eq "$reads" ] && echo 1 || echo 0 )" "" \
    "the operator surface says running=$says_running and a sibling container can read=$reads; a surface that \
disagrees with the mount is the defect this line of work started from" || true
}
...
step_S3_consumers_read() {
  ...
  local holding=0 server
  for server in $P8_SERVERS; do
    if docker exec "$(container_for "$server")" test -d /media/projection >/dev/null 2>&1; then
      holding=$(( holding + 1 ))
    fi
  done
  record "P8-S3-concurrent:$CYCLE_ID" bool "$( [ "$holding" -eq 3 ] && echo 1 || echo 0 )" "" \
    "$holding of 3 servers are holding the same projected path at the same instant" || true
}
...
step_S5_recovery_cycle() {
  # ONE SAFE AUTOMATIC RECOVERY, IN A NAMESPACE THAT IS NOT FRESH. The injector is Phase 7 R1's own — the
  # mount taken out from under a living daemon, lazily because three media servers are holding it — and the
  # assertions are the ones that arm closed on, minus the underlay digest, which belongs to the arm rather
  # than to the soak.
  step_S5_recovery
}
...
step_S6_layers() {
  ...
  record "P8-S6-layers:$CYCLE_ID" le "$layers" "$P8_MOUNT_LAYERS_ABOVE_FLOOR_MAX" \
    "mounts of OURS at the projected mount point above the floor taken before the FIRST cycle mounted \
anything — never a floor re-measured per cycle, which is the reasoning error Phase 7 §11.4 #16 turned on" || true
}
...
step_S7_stop_start() {
  ...
  record "P8-S7-started-again:$CYCLE_ID" bool "$started" "" \
    "a sibling container reads a byte through the mount again after the shipped start" || true
  # AND THE THREE SERVERS WERE NEVER TOUCHED, which is what makes the two above a claim about the appliance
  # rather than about how patiently the gate rebuilt its own world.
  bind_fingerprint "$WORK/out/binds-s7-$CYCLE_ID.txt"
  if cmp -s "$WORK/out/binds-before.txt" "$WORK/out/binds-s7-$CYCLE_ID.txt"; then
    record "P8-S7-consumers-untouched:$CYCLE_ID" bool 1 "" \
      "the same three containers, started at the same instants, holding the same mounts across a stop and a start"
  else
    record "P8-S7-consumers-untouched:$CYCLE_ID" bool 0 "" \
      "a consumer container or one of its mounts changed across the stop and start" || true
    diff "$WORK/out/binds-before.txt" "$WORK/out/binds-s7-$CYCLE_ID.txt" >&2 || true
  fi
}

step_S8_recovery_state() {
  # THE DURABLE LEDGER, READ FROM DISK RATHER THAN INFERRED, and the reset that is the only thing that clears
  # it. A ledger that drifts across cycles is a lockout an operator cannot reason about.
  local ledger attempts_disk attempts_surface
  ledger="$WORK/cache/recovery/recovery-ledger.json"
  attempts_disk=""
  if [ -s "$ledger" ]; then
    attempts_disk="$(node "$REL/out/jq.cjs" attempts < "$ledger" 2>/dev/null || true)"
  fi
  sample
  attempts_surface="${REC_ATTEMPTS:-}"
  record "P8-S8-ledger-truthful:$CYCLE_ID" bool \
    "$( [ -n "$attempts_surface" ] && [ "${attempts_disk:-$attempts_surface}" = "$attempts_surface" ] && echo 1 || echo 0 )" "" \
    "the durable ledger says '${attempts_disk:-absent}' and the shipped surface says '${attempts_surface:-absent}'; \
a surface that disagrees with the file it is derived from is a lockout nobody can reason about" || true
  alpha reset-recovery
  sample
  record "P8-S8-reset-cleared-it:$CYCLE_ID" bool \
    "$( [ "${REC_ATTEMPTS:-x}" = "0" ] && [ "${REC_STATE:-}" = "idle" ] && echo 1 || echo 0 )" "" \
    "after the shipped reset the daemon is '${REC_STATE:-absent}' with ${REC_ATTEMPTS:-?} attempts spent" || true
}
...
step_S9_upgrade_rollback() {
  ...
  record "P8-S9-rollback-returned:$CYCLE_ID" bool "$readable" "" \
    "after the shipped rollback a sibling container reads a byte through the mount again" || true
}

step_S10_cleanup_accounting() {
  # THIS CYCLE'S OWN TRANSIENT RESOURCES, AND NOTHING ELSE. The host's four sets are compared against the
  # snapshot taken before anything of this soak existed — not against the previous cycle, because a leak that
  # is steady across cycles would compare equal every time.
  local containers networks volumes ok=1
  containers="$(docker ps -a --format '{{.Names}}' | LC_ALL=C sort)"
  networks="$(docker network ls --format '{{.Name}}' | LC_ALL=C sort)"
  volumes="$(docker volume ls --format '{{.Name}}' | LC_ALL=C sort)"
  # THE SOAK'S OWN CONTAINERS ARE EXPECTED AND ARE SUBTRACTED BY NAME, never by count.
  local expected
  ...
  record "P8-S10-sets-identical:$CYCLE_ID" bool "$ok" "" \
    "every container on the host is either one the soak declared or one that was there before it started" || true
  record "P8-S10-no-operator-intervention:$CYCLE_ID" le "$OPERATOR_INTERVENTIONS" \
...
# ----------------------------------------------------------------------------------------------------------
record P8-consumers-pre-attached bool "$CONSUMERS_PRE_ATTACHED" "" \
...
CONSUMER_RESTARTS=0
CYCLE_INDEX=0
for CYCLE_INDEX in $(seq 1 "$P8_CYCLES_PER_SOAK"); do
  CYCLE_ID="C$CYCLE_INDEX"
  echo
  echo "############################################################"
  echo "# CYCLE $CYCLE_INDEX of $P8_CYCLES_PER_SOAK"
  echo "############################################################"
  node "$REL/out/cycle.cjs" "$CYCLES_REL" "$CYCLE_INDEX"
  ...
  # THE FULL PER-CYCLE VERIFICATION, WHICH IS THE SAME ONE PHASE 7 RAN AFTER EVERY ARM.
  verify_after_cycle
  record "P8-cycle:$CYCLE_ID" bool 1 "" \
    "cycle $CYCLE_INDEX of $P8_CYCLES_PER_SOAK completed all ten steps in the order §3 names" || true
done

# ----------------------------------------------------------------------------------------------------------
step "THE SOAK'S OWN ASSERTIONS — the ones that are about the WHOLE soak rather than any cycle"
# ----------------------------------------------------------------------------------------------------------
# THE CONSUMERS WERE NEVER TOUCHED, COUNTED ACROSS THE WHOLE SOAK. This is the threshold this tranche adds,
# and it is counted rather than inferred: the gate never restarts one, so any nonzero count is a defect in the
# gate or a container the host restarted underneath it, and either is a soak that did not soak.
for server in $P8_SERVERS; do
  container="$(container_for "$server")"
  restarts="$(docker inspect -f '{{.RestartCount}}' "$container" 2>/dev/null || echo "")"
  case "$restarts" in
    ''|*[!0-9]*) CONSUMER_RESTARTS=$(( CONSUMER_RESTARTS + 1 )) ;;
    *) CONSUMER_RESTARTS=$(( CONSUMER_RESTARTS + restarts )) ;;
  esac
done
record P8-consumers-never-touched le "$CONSUMER_RESTARTS" "$P8_CONSUMER_RESTARTS_MAX" \
  "restarts across all three media servers for the whole soak, read from Docker's own counter" || true
...
leak_scan P8-leak-manifest    "the published manifest directory" "$WORK/manifest" \
...
if node "$REL/out/refplacement.cjs" "$OBJECTS_FILE" "$REL/manifest"; then
  record P8-leak-manifest-ref-placement bool 1 "" \
    "every occurrence of the stable reference in the manifest IS a locator.objectRef value"
else
  record P8-leak-manifest-ref-placement bool 0 "" \
...
leak_scan P8-leak-probe-cache "the daemon probe cache"           "$WORK/cache"
for scan_dir in jf-config plex-config emby-config; do
  docker run --rm -v "$WORK/$scan_dir:/scan:ro" -v "$WORK/out:/out:ro" "$VERIFY_IMAGE" \
    ...
    || { record P8-leak-library-state bool 0 "" "a media server persisted a secret, reference or label" \
         || true; die "a media server's library state holds a secret, a reference or the operator's label"; }
done
record P8-leak-library-state bool 1 "" "all three servers' library state, searched in full"
...
leak_scan P8-leak-evidence "the preserved evidence" "$EVIDENCE_DIR"
...
record P8-resolutions-happened ge "${RESOLUTIONS:-0}" 1 \
...
record P8-own-mountpoints-removed eq "${LEFT_MOUNTS:-1}" 0 \
  "mountpoints left under this run's own directory" || true
record P8-own-run-directory-removed bool "$( [ ! -d "$WORK" ] && echo 1 || echo 0 )" "" \
...
compare_sets P8-host-container-set-unchanged "containers" \
  "$GATE_ROOT/host-containers-before-$$.txt" "$GATE_ROOT/host-containers-after-$$.txt"
compare_sets P8-host-network-set-unchanged "networks" \
  "$GATE_ROOT/host-networks-before-$$.txt" "$GATE_ROOT/host-networks-after-$$.txt"
compare_sets P8-host-volume-set-unchanged "volumes" \
...
npx tsx src/ops/projection-phase8-cli.ts redaction-check --file "$RESULTS_REL" \
  || die "the preserved verdict log is not redaction-safe"
npx tsx src/ops/projection-phase8-cli.ts report --results "$RESULTS_REL" --cycles-log "$CYCLES_REL"
npx tsx src/ops/projection-phase8-cli.ts close --results "$RESULTS_REL" --cycles-log "$CYCLES_REL" \
...
npx tsx src/ops/projection-phase8-cli.ts nonclaims
