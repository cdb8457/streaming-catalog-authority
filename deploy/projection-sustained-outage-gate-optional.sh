#!/usr/bin/env bash
# The sustained-outage gate for a host where it is OPTIONAL.
#
...
# evidence. `npm run go:sustained-outage-gate` and `npm run go:sustained-outage-gate:three` propagate 77, so a
...
GATE_SKIP_STATUS=77
GATE_COMMAND="${PROJECTION_SUSTAINED_OUTAGE_GATE_COMMAND:-$HERE/projection-sustained-outage-gate.sh}"
...
if [ "$status" -eq "$GATE_SKIP_STATUS" ]; then
  echo >&2
  echo "OPTIONAL HOST: the sustained-outage gate skipped (status ${GATE_SKIP_STATUS}) and this entry point treats" >&2
  echo "that as success. NOTHING WAS PROVED. No acceptance gate is closed by this run. Use" >&2
  echo "'npm run go:sustained-outage-gate' on a host that can host it for evidence." >&2
  exit 0
fi
exit "$status"
