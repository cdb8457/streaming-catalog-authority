#!/usr/bin/env bash
# The STALE-MOUNT GATE, three times: a runner that proves the gate is repeatable, not lucky.
#
# WHY THREE. A stale mount is a corpse — a mountpoint whose daemon was SIGKILLed and never came back — and a
# remount over it is a kernel conversation that repeats exactly on the same host, so the flake a single run
# cannot see is the one that only shows up when the conversation happens again. Three clean runs with every
# run asserted the same way is the difference between "it passed on a good day" and "it passes".
#
...
# runs: each run is a cold start of PostgreSQL and the daemon, exactly as `npm run go:stale-mount-gate`
...
GATE="$HERE/projection-stale-mount-gate.sh"
...
if docker ps --filter "name=projection-stale-mount-" --format '{{.Names}}' | grep -q .; then
  echo "REFUSING TO RUN: a previous stale-mount container is still alive." >&2
...
for round in 1 2 3; do
  echo
  echo "==== stale-mount gate, run $round/3 ===="
  set +e
  START="$(date +%s%3N)"
  bash "$GATE"
  RC=$?
...
echo
echo "stale-mount gate: ${PASS} pass, ${FAIL} fail${TIMES}"
if [ "$FAIL" -ne 0 ]; then
